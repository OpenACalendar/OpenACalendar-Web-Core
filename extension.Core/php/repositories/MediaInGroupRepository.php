...
class MediaInGroupRepository{
	...
	public function add(MediaModel $media, GroupModel $group, UserAccountModel $user) {
		...
		// Add!
		$stat = $DB->prepare("INSERT INTO media_in_group (group_id,media_id,added_by_user_account_id,added_at) ".
				"VALUES (:group_id,:media_id,:added_by_user_account_id,:added_at)");
		$stat->execute(array(
			'group_id'=>$group->getId(),
			'media_id'=>$media->getId(),
