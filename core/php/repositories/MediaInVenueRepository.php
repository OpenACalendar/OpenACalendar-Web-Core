...
class MediaInVenueRepository{
	...
	public function add(MediaModel $media, VenueModel $venue, UserAccountModel $user) {
		...
		// Add!
		$stat = $DB->prepare("INSERT INTO media_in_venue (venue_id,media_id,added_by_user_account_id,added_at) ".
				"VALUES (:venue_id,:media_id,:added_by_user_account_id,:added_at)");
		$stat->execute(array(
			'venue_id'=>$venue->getId(),
			'media_id'=>$media->getId(),
